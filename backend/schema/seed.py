...
from app.modules.plantas.models import Plant, Area, UserPlantAccess, UserAreaAccess
...
def run_seed():
    ...
    try:
        ...
        estadisticas = {
            "empresas": 0, "usuarios": 0, "plantas": 0,
            "areas": 0, "maquinas": 0, "maquinas_seguras": 0, "firmas_legales": 0
        }
        ...
        print(f"   🛡️  Firmas Digitales (Legal):  {estadisticas['firmas_legales']}")
...
def _clear_data(db: Session):
    """
    Limpia todas las tablas en orden correcto respetando las FK.
    FIX: eliminados los DELETE raw de user_plant_access y user_area_access
    — ahora se borran via ORM junto al resto, en el orden correcto.
    """
    print("🧹 Purgando registros y bitácoras anteriores...")

    db.execute(text("DELETE FROM machine_pin_audit_logs"))
    db.execute(text("DELETE FROM document_read_confirmations"))
    db.execute(text("DELETE FROM maintenance_activity_logs"))
    db.execute(text("DELETE FROM maintenance_tasks"))
    db.execute(text("DELETE FROM documents"))
    db.execute(text("DELETE FROM machines"))
    # FIX: user_area_access y user_plant_access ahora tienen id y is_active
    # Se borran con DELETE simple — compatible con la nueva estructura
    db.execute(text("DELETE FROM user_area_access"))
    db.execute(text("DELETE FROM user_plant_access"))
    db.execute(text("DELETE FROM areas"))
...
def _crear_usuarios_jerarquicos(
    db: Session, company: Company, domain: str, is_first: bool
) -> dict:
...
def _crear_plantas_y_accesos(
    db: Session, company: Company, users: dict
    ...
    for i in range(1, 4):
        ...
        # FIX: INSERT via ORM usando el modelo UserPlantAccess
        # Ahora tiene id (UUID autogenerado), is_active y updated_at
        for user in (jefe_planta, tecnico_libre):
            db.add(UserPlantAccess(user_id=user.id, plant_id=p.id))
        ...
        for j, tipo in enumerate(tipos_areas, 1):
            ...
            # FIX: INSERT via ORM usando el modelo UserAreaAccess
            for user in (jefe_area, tecnico_fijo):
                db.add(UserAreaAccess(user_id=user.id, area_id=a.id))
...
def _crear_maquinas_seguras(
    db: Session, plantas: list[Plant], users: dict
        ...
        for area in planta.areas:
            for i in range(1, 10):
                ...
                if req_pin:
                    ...
                    db.execute(
                        text(
                            "INSERT INTO machine_pin_audit_logs "
                            "(machine_id, changed_by_id, action) "
                            "VALUES (:m, :u, 'PIN_CREATED')"
                        ),
...
def _generar_comportamiento_operativo(
    db: Session, company: Company, maquinas: list[Machine], users: dict
) -> int:
    now            = datetime.utcnow()
    admin          = users["admin"]
    ...
    for m in maquinas:
        ...
        # 2. Escudo legal — operario acepta riesgos y firma digitalmente
        ...
        # 3. Tareas según estado de la máquina
        if m.status == MachineStatus.fallas:
            ...
            # FIX: MaintenanceType. estaba incompleto → corregido a bloqueo_loto
            tarea = MaintenanceTask(
                title=f"Reparación Crítica (Protocolo LOTO) - {m.serial_number}",
                maintenance_type=MaintenanceType.bloqueo_loto,
